...
def led_gauge(value: float, label: str, bad_th: float, warn_th: float, segments: int = 28):
    ...
    fig.add_trace(
        go.Scatterpolar(
            ...
            marker=dict(
                ...
                line=dict(color=_rgba(NEON, 0.40), width=3),
            ),
            hoverinfo="skip",
        )
    )
    ...
    fig.add_annotation(
        ...
        xref="paper",
        yref="paper",
        text=f"<span style='font-size:13px; color:rgba(230,245,255,0.72); letter-spacing:0.6px; text-transform:uppercase;'>{label}</span>",
        showarrow=False,
    )
...
def pareto_chart(df: pd.DataFrame, col: str, title: str):
    ...
    fig.update_layout(
        ...
        paper_bgcolor="rgba(0,0,0,0)",
        plot_bgcolor="rgba(35,40,48,0.18)",
...
def load_from_repo() -> dict:
    ...
    for col in ["Causa", "Ação", "Indicador", "Setor", "Responsável", "Prazo", "Status", "Observações"]:
...
f = table.copy()
if "Status" in f.columns and f_status:
    f = f[f["Status"].isin(f_status)]
if "Setor" in f.columns and f_setor:
    f = f[f["Setor"].astype(str).isin(f_setor)]
if "Responsável" in f.columns and f_resp:
    f = f[f["Responsável"].astype(str).isin(f_resp)]
if "Indicador" in f.columns and f_ind:
...
with g1:
    render_hud(
        led_gauge(m["health"], "Saúde do PA", 50, 75, 28),
        "Velocímetro LED • Saúde do PA",
        subtitle=f"Penalidade por atrasos: {m['penalty']:.1f} pts",
    )
with g2:
    render_hud(
        led_gauge(m["completion"], "Conclusão do PA", 40, 70, 28),
...
if not setores_current:
    ...
    with s1:
        render_hud(
            led_gauge(ms["health"], f"Saúde • {sel}", 50, 75, 28),
            f"Setor: {sel} • Saúde (LED)",
            subtitle=f"Atrasos: {ms['overdue']} • Penalidade: {ms['penalty']:.1f} pts",
        )
    with s2:
        render_hud(
            led_gauge(ms["completion"], f"Conclusão • {sel}", 40, 70, 28),
