...
def led_gauge(value: float, label: str, bad_th: float, warn_th: float, segments: int = 26):
    ...
    start_deg = 210
    sweep_deg = 240
    theta = np.linspace(start_deg, start_deg - sweep_deg, segments, endpoint=False)
    width = sweep_deg / segments

    seg_vals = np.linspace(0, 100, segments, endpoint=False)
    active = seg_vals < v
    ...
    fig.add_trace(go.Barpolar(
        r=np.full(segments, thickness),
        theta=theta,
        width=np.full(segments, width*0.92),
        base=np.full(segments, r_inner),
    ...
    fig.add_trace(go.Barpolar(
        r=np.full(segments, thickness*0.98),
        theta=theta,
        width=np.full(segments, width*0.92),
        base=np.full(segments, r_inner),
    ...
    fig.add_trace(go.Barpolar(
        r=np.full(segments, thickness*1.06),
        theta=theta,
        width=np.full(segments, width*0.86),
        base=np.full(segments, r_inner - 0.01),
