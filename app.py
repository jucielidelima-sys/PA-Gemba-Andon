...
.hud:after{
  ...
  position:absolute;
  inset:-20px;
  background: radial-gradient(circle at 50% 50%, rgba(0,229,255,0.10), rgba(0,0,0,0) 60%);
  filter: blur(1px);
  pointer-events:none;
}
.hud-title{
...
def led_gauge(value: float, label: str, bad_th: float, warn_th: float, segments: int = 26):
    ...
    zone_colors = []
    for sv in seg_vals:
        if sv < bad_th: zone_colors.append(BAD)
        elif sv < warn_th: zone_colors.append(WARN)
        else: zone_colors.append(GOOD)
    ...
    r_ring = [thickness] * segments
    base = [r_inner] * segments
    w92 = [width * 0.92] * segments
    w86 = [width * 0.86] * segments

    fig = go.Figure()
    fig.add_trace(go.Barpolar(
        r=r_ring, theta=theta, width=w92, base=base,
        marker_color=inactive_cols, marker_line_color="rgba(255,255,255,0.06)",
        marker_line_width=1, hoverinfo="skip",
    ))
    fig.add_trace(go.Barpolar(
        r=[thickness*0.98]*segments, theta=theta, width=w92, base=base,
        marker_color=active_cols_zone, marker_line_color="rgba(255,255,255,0.00)",
        hoverinfo="skip",
    ))
    fig.add_trace(go.Barpolar(
        r=[thickness*1.06]*segments, theta=theta, width=w86, base=[r_inner-0.01]*segments,
        marker_color=active_cols_neon, marker_line_color=_rgba(NEON, 0.22),
        marker_line_width=1, hoverinfo="skip",
    ))
    ...
    fig.add_annotation(
        x=0.5, y=0.29, xref="paper", yref="paper",
...
<div class="hud">
  ...
  <div style="height:{height-88}px; margin-top:-10px;">
    {html}
  </div>
</div>
""",
...
with g1:
    render_hud(
        led_gauge(m["health"], "Saúde do PA", bad_th=50, warn_th=75, segments=26),
        "Velocímetro LED • Saúde do PA",
        subtitle=f"Painel digital • Penalidade: {m['penalty']:.1f} pts",
    )
with g2:
    render_hud(
        led_gauge(m["completion"], "Conclusão do PA", bad_th=40, warn_th=70, segments=26),
...
if not setores_current:
    ...
    with s1:
        render_hud(
            led_gauge(ms["health"], f"Saúde • {sel}", bad_th=50, warn_th=75, segments=26),
            f"Setor: {sel} • Saúde (LED)",
            subtitle=f"Atrasos: {ms['overdue']} • Penalidade: {ms['penalty']:.1f} pts",
        )
    with s2:
        render_hud(
            led_gauge(ms["completion"], f"Conclusão • {sel}", bad_th=40, warn_th=70, segments=26),
