...
# -------------------------------------------------------
# Estilo: indústria 4.0 / andon + glassmorphism
# -------------------------------------------------------
st.markdown(
    """
<style>
...
.big-title{
  ...
  text-shadow: 0 0 22px rgba(0,229,255,0.28);
}
.subtitle{
  color:var(--muted);
  font-size:12px;
...
.gauge-glass:before{
  ...
  height:180px;
  border-radius:999px;
  background: radial-gradient(circle, rgba(0,229,255,0.28), rgba(0,0,0,0));
  filter: blur(2px);
}
.gauge-glass:after{
  /* anel de brilho */
...
def render_gauge_glass(fig, title: str, height: int = 360):
...
# -------------------------------------------------------
# Gauges: geral no topo + por setor (dropdown)
# -------------------------------------------------------
g1, g2 = st.columns(2)
with g1:
    render_gauge_glass(
        gauge_fig(m["health"], "Saúde do PA", threshold_bad=50, threshold_warn=75),
        "Velocímetro Geral • Saúde do PA",
        height=380,
    )
...
with g2:
    render_gauge_glass(
        gauge_fig(m["completion"], "Conclusão do PA", threshold_bad=40, threshold_warn=70),
        "Velocímetro Geral • Conclusão",
        height=380,
    )
...
# Dropdown: setores do recorte atual (para não ficar com setores vazios)
...
if not setores_current:
    ...
    with s1:
        render_gauge_glass(
            gauge_fig(ms["health"], f"Saúde • {sel}", threshold_bad=50, threshold_warn=75),
            f"Setor: {sel} • Saúde",
            height=380,
        )
        st.markdown(
            f'<div class="small-note">Atrasos no setor: <b>{ms["overdue"]}</b> • Penalidade: <b>{ms["penalty"]:.1f}</b> pts</div>',
            unsafe_allow_html=True,
        )
    with s2:
        render_gauge_glass(
            gauge_fig(ms["completion"], f"Conclusão • {sel}", threshold_bad=40, threshold_warn=70),
