...
ROBOTIC_RIGHT = ASSETS_DIR / "robotic_right.png"
...
[data-testid="stToolbar"]{
  background:
    radial-gradient(900px 380px at 20% 0%, rgba(255,255,255,0.06), rgba(0,0,0,0) 60%),
    linear-gradient(180deg, #6a707a 0%, #565d67 38%, #434a55 100%) !important;
...
[data-testid="stSidebarContent"]{
  background: linear-gradient(180deg, rgba(78,86,98,0.98), rgba(56,62,73,0.98));
...
:root{
  ...
  --info: #7ABEFF;
  --safety: #FFE06A;
  --panel: rgba(54,63,76,0.82);
  --panel2: rgba(41,48,59,0.88);
}
...
div[data-baseweb="select"]{
  background: linear-gradient(180deg,#7f8895,#606977) !important;
...
div[data-testid="stDataFrame"]{
  background: linear-gradient(180deg, rgba(110,118,130,0.96), rgba(75,83,95,0.96)) !important;
...
.titlebar{
  ...
  padding:16px 18px;
  background:
    radial-gradient(900px 260px at 0% 0%, rgba(73,207,255,0.14), rgba(0,0,0,0)),
    linear-gradient(180deg, rgba(75,83,95,0.86), rgba(50,57,68,0.86));
  position:relative;
  overflow:hidden;
}
.titlebar:after{
...
.kpi{
  ...
  padding:14px 16px;
  background:
    radial-gradient(500px 180px at 0% 0%, rgba(73,207,255,0.10), rgba(0,0,0,0)),
...
.hud{
  ...
  background:
    radial-gradient(520px 220px at 0% 0%, rgba(73,207,255,0.12), rgba(0,0,0,0)),
    radial-gradient(520px 220px at 100% 100%, rgba(255,224,106,0.08), rgba(0,0,0,0)),
    rgba(63,72,84,0.78);
  ...
  overflow: hidden;
  box-shadow: 0 0 0 1px rgba(73,207,255,0.08) inset, 0 16px 34px rgba(0,0,0,0.35);
}
.hud:before{
  content:"";
  position:absolute;
...
.section-card{
  ...
  border-radius:18px;
  background: linear-gradient(180deg, rgba(80,88,100,0.74), rgba(56,63,74,0.78));
  padding:12px;
}
...
.right-robot{
  ...
  overflow: hidden;
  background:
    linear-gradient(180deg, rgba(101,108,120,0.12), rgba(63,70,82,0.28)),
...
.right-robot::before{
  ...
  inset:0;
  background:
    radial-gradient(500px 900px at 20% 45%, rgba(255,255,255,0.08), rgba(0,0,0,0) 55%),
    linear-gradient(90deg, rgba(101,108,120,0.05) 0%, rgba(101,108,120,0.02) 35%, rgba(101,108,120,0.00) 100%);
  pointer-events:none;
  z-index:1;
}
.right-robot img{
...
.pareto-panel{
  border:1px solid rgba(255,255,255,0.14);
  border-radius:18px;
  background: linear-gradient(180deg, rgba(78,86,98,0.52), rgba(56,62,73,0.60));
...
def pareto_chart(df: pd.DataFrame, col: str, title: str):
    ...
    fig.add_trace(go.Bar(
        ...
        marker=dict(color=AI_BLUE, line=dict(color="#BFF3FF", width=1.2)),
        opacity=0.96
    ))
    ...
    fig.update_layout(
        ...
        plot_bgcolor="rgba(60,68,80,0.12)",
...
g1, g2 = st.columns(2)
with g1:
    render_hud(led_gauge(m["health"], "Saúde do PA", 50, 75, 28), "Velocímetro LED • Saúde do PA",
               subtitle=f"Penalidade por atrasos: {m['penalty']:.1f} pts")
with g2:
    render_hud(led_gauge(m["completion"], "Conclusão do PA", 40, 70, 28), "Velocímetro LED • Conclusão do PA",
...
if not setores_current:
    ...
    s1, s2 = st.columns(2)
    with s1:
        render_hud(led_gauge(ms["health"], f"Saúde • {sel}", 50, 75, 28), f"Setor: {sel} • Saúde (LED)",
                   subtitle=f"Atrasos: {ms['overdue']} • Penalidade: {ms['penalty']:.1f} pts")
    with s2:
        render_hud(led_gauge(ms["completion"], f"Conclusão • {sel}", 40, 70, 28), f"Setor: {sel} • Conclusão (LED)",
                   subtitle="Monitoramento turno a turno")

st.divider()

main_col, right_col = st.columns([1.0, 0.42], gap="medium")
...
with right_col:
    if ROBOTIC_RIGHT.exists():
        img_b64 = base64.b64encode(ROBOTIC_RIGHT.read_bytes()).decode("utf-8")
        components.html(
            f'''
            <div class="right-robot" style="position:sticky; top:8px; height:1120px;">
              <img src="data:image/png;base64,{img_b64}" alt="robotic visual"/>
            </div>
            ''',
            height=1130,
            scrolling=False,
