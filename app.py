...
# -------------------------------------------------------
# Estilo: preto mais "fraco" (grafite) + tabela preta + Kanban
# -------------------------------------------------------
st.markdown(
    """
<style>
/* Tom grafite (menos preto chapado) */
html, body, .stApp,
...
[data-testid="stSidebarContent"]{
  background:#0b0f1a !important; /* grafite */
}
...
.block-container{padding-top:1.0rem; padding-bottom:2.2rem; max-width: 1600px;}
section[data-testid="stSidebar"]{border-right:1px solid var(--line);}

/* Deixar a área de tabela (st.dataframe) preta/grafite também */
...
.titlebar{
  ...
  padding:16px 18px;
  background:
    radial-gradient(900px 260px at 0% 0%, rgba(0,229,255,0.16), rgba(0,0,0,0)),
    radial-gradient(700px 220px at 100% 0%, rgba(255,214,10,0.14), rgba(0,0,0,0)),
    linear-gradient(180deg, rgba(18,28,51,0.98), rgba(11,15,26,0.90));
  position: relative;
  overflow:hidden;
}
.titlebar:after{
...
.dot.neon{background:var(--neon); box-shadow: 0 0 10px rgba(0,229,255,0.65);}
...
.gauge-glass:before{
  ...
  height:220px;
  border-radius:999px;
  background: radial-gradient(circle, rgba(0,229,255,0.30), rgba(0,0,0,0));
  filter: blur(3px);
}
.gauge-glass:after{
  content:"";
  ...
  box-shadow:
    0 0 22px rgba(0,229,255,0.26),
    0 0 65px rgba(0,229,255,0.11);
  pointer-events:none;
}
.gauge-title{
  font-weight:900;
...
# Kanban (igual conceito do print)
...
def render_col(df, container, title, dot, status_value):
    with container:
        ...
        for _, r in subset.head(10).iterrows():
            ...
            line2 = " • ".join([x for x in [setor, resp, (f"{dias:+}d" if isinstance(dias, int) else "")] if x and x != "nan"])
            st.markdown(
                f"""
