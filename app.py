...
[data-testid="stToolbar"]{
  background:
    ...
    linear-gradient(180deg, #656c78 0%, #4f5662 38%, #3f4652 100%) !important;
...
.right-robot{
  ...
  border-radius:22px;
  overflow:hidden;
  background: linear-gradient(180deg, rgba(85,93,106,0.66), rgba(58,65,76,0.76));
...
main_col, right_col = st.columns([1.0, 0.28], gap="large")
...
with right_col:
    if ROBOTIC_RIGHT.exists():
        st.markdown('<div class="right-robot">', unsafe_allow_html=True)
        st.image(str(ROBOTIC_RIGHT), use_container_width=True)
        st.markdown('</div>', unsafe_allow_html=True)
