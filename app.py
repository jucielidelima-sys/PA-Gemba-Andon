...
[data-testid="stToolbar"]{
  background: linear-gradient(180deg, #0B2A52 0%, #0A2040 48%, #081A33 100%) !important;
...
.big-title{
  ...
  text-shadow:0 0 22px rgba(117,233,255,0.35);
}
...
.robot-side{
  ...
  border-radius:18px;
  overflow:hidden;
  background: linear-gradient(180deg, rgba(18,49,93,0.36), rgba(10,32,64,0.46));
  min-height: 500px;
}
.robot-side img{
  width:100%;
  ...
  object-fit:cover;
  display:block;
  filter: saturate(1.55) hue-rotate(-25deg) contrast(1.18);
  opacity:0.88;
}
...
.pareto-panel{
  border:1px solid rgba(140,220,255,0.16);
  border-radius:18px;
  background: linear-gradient(180deg, rgba(18,49,93,0.22), rgba(10,32,64,0.30));
...
def pareto_chart(df: pd.DataFrame, col: str, title: str):
    ...
    fig.add_trace(go.Bar(
        ...
        marker=dict(color=AI_BLUE, line=dict(color="#B7F1FF", width=1.2)),
        opacity=0.96
    ))
    ...
    fig.update_layout(
        ...
        plot_bgcolor="rgba(11,42,82,0.18)",
...
# layout com braços laterais em colunas reais, sem hacks de CSS/JS
side_left, main_center, side_right = st.columns([0.20, 1.0, 0.20], gap="medium")

with side_left:
    if ROBOTIC_BG.exists():
        st.markdown('<div class="robot-side">', unsafe_allow_html=True)
        st.image(str(ROBOTIC_BG), use_container_width=True)
        st.markdown('</div>', unsafe_allow_html=True)

with main_center:
    left, right = st.columns([1.05, 0.95])
    with left:
        st.markdown("<div class='robot-section'>", unsafe_allow_html=True)
