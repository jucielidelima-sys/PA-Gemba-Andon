...
# -------------------------------------------------------
# Visual: fundo preto + letras brancas + padrão fábrica 4.0 (HUD/ANDON)
# -------------------------------------------------------
st.markdown(
    """
<style>
/* Fundo 100% preto em tudo */
html, body, .stApp,
...
.block-container{padding-top:1.0rem; padding-bottom:2.2rem; max-width: 1650px;}
section[data-testid="stSidebar"]{border-right:1px solid var(--line);}

/* Dataframe: manter escuro */
...
/* Painel velocímetro estilo HUD (fábrica 4.0) */
.hud{
  border-radius: 22px;
  border: 1px solid rgba(255,255,255,0.12);
  background:
...
.hud:after{
  ...
  inset:-20px;
  background:
    radial-gradient(circle at 50% 50%, rgba(0,229,255,0.10), rgba(0,0,0,0) 60%);
  filter: blur(1px);
  pointer-events:none;
}
.hud-title{
...
# -------------------------------------------------------
# Velocímetro: padrão fábrica 4.0 (neon + HUD)
# -------------------------------------------------------
def gauge_hud(value: float, title: str, threshold_bad: float, threshold_warn: float):
    value = max(0.0, min(100.0, float(value)))
    fig = go.Figure(go.Indicator(
        mode="gauge+number",
        value=value,
        number={"suffix": "%", "font": {"size": 56, "color": "rgba(255,255,255,0.96)"}},
        ...
        gauge={
            ...
            "axis": {
                ...
                "tickfont": {"size": 12, "color": "rgba(255,255,255,0.75)"},
            },
            "bar": {"color": "rgba(0,229,255,0.95)", "thickness": 0.34},
            "bgcolor": "rgba(0,0,0,0)",
            "borderwidth": 1,
            "bordercolor": "rgba(255,255,255,0.14)",
            "steps": [
                {"range": [0, threshold_bad], "color": "rgba(255,23,68,0.20)"},
                {"range": [threshold_bad, threshold_warn], "color": "rgba(255,145,0,0.18)"},
                {"range": [threshold_warn, 100], "color": "rgba(0,200,83,0.14)"},
            ],
            "threshold": {
                "line": {"color": "rgba(255,214,10,1.0)", "width": 6},
                "thickness": 0.82,
                "value": value,
            }
        }
    ...
    # Anel neon extra (shape) para parecer "4.0"
    fig.update_layout(
        height=340,
        margin=dict(l=14, r=14, t=10, b=0),
        paper_bgcolor="rgba(0,0,0,0)",
        plot_bgcolor="rgba(0,0,0,0)",
        font=dict(color="rgba(255,255,255,0.92)"),
        shapes=[
            # arco superior (efeito neon) - aproximado com círculo
            dict(type="circle", xref="paper", yref="paper",
                 x0=0.14, y0=0.06, x1=0.86, y1=0.78,
                 line=dict(color="rgba(0,229,255,0.22)", width=6))
        ]
    )
    return fig

def render_hud(fig, title: str, subtitle: str = "", height: int = 420):
...
<div class="hud">
  ...
  <div style="height:{height-88}px; margin-top:-6px;">
    {html}
  </div>
</div>
""",
...
# Velocímetros (HUD/4.0)
g1, g2 = st.columns(2)
with g1:
    render_hud(
        gauge_hud(m["health"], "Saúde do PA", 50, 75),
        "Velocímetro • Saúde do PA",
        subtitle=f"Penalidade por atrasos: {m['penalty']:.1f} pts",
        height=430,
    )
with g2:
    render_hud(
        gauge_hud(m["completion"], "Conclusão do PA", 40, 70),
        "Velocímetro • Conclusão do PA",
...
if not setores_current:
    ...
    with s1:
        render_hud(
            gauge_hud(ms["health"], f"Saúde • {sel}", 50, 75),
            f"Setor: {sel} • Saúde",
    ...
    with s2:
        render_hud(
            gauge_hud(ms["completion"], f"Conclusão • {sel}", 40, 70),
            f"Setor: {sel} • Conclusão",
